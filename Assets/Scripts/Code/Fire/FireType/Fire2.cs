    ...
    public class Fire2 : Fire
    {
        ...
        protected override void DoDestroy()
        {
            if (_life > 0)
            {
                _life--;
                _slider.value -= 1;
                //if (_fireImage.localScale.x > .75f) _fireImage.localScale -= new Vector3(_valEscala, _valEscala, 0);
                //_fireImage.GetChild(0).localScale -= new Vector3(_valEscala, _valEscala, 0);
                return;
            }
            if (transform.GetChild(1).GetComponentInChildren<Image>().color != Color.clear) FireController._totalFires--;
        ...
        protected override void DoStart()
        {
            ...
            _fireCamMap = _fireImage2.GetComponent<SpriteRenderer>();
            ...
            //if(_constantValueMultiplier != 6.2f / 1.75f) _constantValueMultiplier = 6.2f / 1.75f;
            _constantValueMultiplier = 6.2f / 1.75f;
            _spriteFace = _fireImage.GetChild(0).GetComponent<SpriteRenderer>();
        }
        public void ChangeConstantValueMultiplier(float value)
        {
            _constantValueMultiplier = value;
        }
        public float GetConstantValueMultiplier()
        {
