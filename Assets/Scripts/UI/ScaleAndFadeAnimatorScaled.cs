...
public class ScaleAndFadeAnimatorScaled : MonoBehaviour
{
    ...
    private void Update()
    {
        if (animating)
        {
            timeElapsed += Time.deltaTime; // Usar Time.unscaledDeltaTime para animar incluso cuando el juego está en pausa
            ...
            foreach (Transform trans in allTransforms)
            {
                ...
                trans.localScale = Vector3.Lerp(Vector3.zero, currentScale, scaleValue);
            }
